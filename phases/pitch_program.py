...
def load_atmosphere_table(path="kerbin_atmosphere_table.json"):
    ...
    return alt_table, rho_table, p_table
...
@njit(nogil=True)
def jit_vertical_accel(r_vec, v_vec, mass, thrust_magnitude, GM):
    r_unit = r_vec / np.linalg.norm(r_vec)
    a_thrust = (thrust_magnitude / mass) * r_unit
    a_gravity = -(GM / np.dot(r_vec, r_vec)) * r_unit
    return a_thrust + a_gravity
...
class PitchProgram():
    def __init__(self, vessel, conn):
        ...
        self.alt_table, self.rho_table, self.p_table = load_atmosphere_table()
    ...
    # ──────────────────────────────────────────────────────────────────
    # Precompute (runs BEFORE the rocket actually starts flying)
    # ──────────────────────────────────────────────────────────────────

    def begin_precompute(self, target_apoapsis, vertical_ascent_altitude=300.0):
        """Launch the FIRST optimization in a background thread, using a
        physics-projected end-of-vertical-ascent state instead of live
        telemetry. Runs concurrently with the real vertical_ascent phase.

        `start_time` is deliberately left unset — run_program() anchors
        it to the exact real moment vertical ascent finishes, so the
        pitch schedule's t=0 always matches when the gravity turn
        actually begins, regardless of how long vertical ascent took.
        """
        pad_state = self.Step_1n2()

        self.optimizer_running = True
        print("[PRECOMPUTE] Running initial optimization...")
        ...
        print(f"[PRECOMPUTE] Launched (projecting to {vertical_ascent_altitude:.0f}m)…")

    def _precompute_worker(self, pad_state, target_apoapsis, vertical_ascent_altitude):
        try:
            projected_state = self._project_vertical_ascent_end(
                pad_state, vertical_ascent_altitude
            )
            proj_alt = float(np.linalg.norm(projected_state["position"][0]) - self.radius_kerbin)
            if proj_alt < vertical_ascent_altitude - 50.0:
                print(f"[PRECOMPUTE] ERROR: vertical-climb projection stalled at "
                      f"{proj_alt:.0f} m (target {vertical_ascent_altitude:.0f} m) — "
                      f"fuel model is likely wrong for this vessel. REFUSING to "
                      f"optimize a dead state.")
                return
            ...
            with self.trajectory_lock:
                self.active_trajectory = {
                    ...
                    "start_time": None,  # patched in by run_program
    ...
    def maybe_reoptimize(self, target_apoapsis, last_opt_launch):
        now = time.time()
        if (not self.optimizer_running
                and (now - last_opt_launch) >= self.optimizer_period):
            state_snap = self.Step_1n2()
            if state_snap is not None:
                self.optimizer_running = True
                opt_start = time.time()
                self._opt_thread = threading.Thread(
                    target=self.optimizer_worker,
                    args=(state_snap, target_apoapsis, opt_start),
                    daemon=True,
                )
                self._opt_thread.start()
                print("  [OPT] Optimizer thread launched")
                return opt_start
        return last_opt_launch
    ...
    def run_program(self, target_apoapsis):
        self.target_apoapsis_m = target_apoapsis
        ...
        # ── Use precomputed trajectory if begin_precompute() was called,
        ...
        while True:
            ...
            # maybe_reoptimize section header for why).
            # last_opt_launch = self.maybe_reoptimize(target_apoapsis, last_opt_launch)
    ...
    def TWR_control(self, target_TWR):
        r = np.linalg.norm(np.array(self.position()))
        gravity = self.GM / r**2
        TWR = self.thrust() / (self.mass() * gravity)
        if TWR > target_TWR:
            target_thrust = target_TWR * self.mass() * gravity
            self.vessel.control.throttle = target_thrust / self.thrust()
        else:
            self.vessel.control.throttle = 1.0
    ...
    def Step_1n2(self):
        ...
        try:
            tas = float(self.vessel.flight().true_air_speed)
